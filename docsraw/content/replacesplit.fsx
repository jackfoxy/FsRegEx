...
The `System.Text.RegularExpressions.Regex` Class provides several overloads of `replace`. The same overloads are available in the `FsRegEx` class and as individually 
...
The `System.Text.RegularExpressions.Regex` Class provides several overloads of `split` to split strings into arrays. The same overloads are available in the `FsRegEx` class and as individually 
named functions, including overloads and functions using `RegexOptions`. `Timeout` is not supported at this time.
